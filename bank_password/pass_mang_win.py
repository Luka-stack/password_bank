...
def loginWindow():
    layout = [
        ...
        [sg.T(' ' * 4), sg.Button('Sign Up', size=(8, 1), **btn_font), 
         sg.T(' ' * 5), sg.Button('Log In',  size=(8, 1), **btn_font)],
        [sg.T(' ' * 1), sg.Button('Exit', size=(24, 1), **btn_font)]
    ]
...
def signUp(login, passwd):
    ...
    subprocess.check_call(['attrib', '+H', USERS_DB])
    cursor = conn.cursor()
...
def createLeftColumn(data):
    ...
    for k in data.keys():
        acc_row = [sg.Frame(title=k.capitalize(), relief=sg.RELIEF_SOLID, font=('italic'),
                            layout=[[sg.Button('Show',   key='sh_' + k, size=(7, 1), **btn_font),
                                     sg.Button('Change', key='ch_' + k, size=(7, 1), **btn_font),
                                     sg.Button('Delete', key='dl_' + k, size=(7, 1), **btn_font)]]
                            )
                ]
...
def createRightColumn():
    right_column = [
        ...
        [sg.T(' ' * 6),  sg.Button('Add New Account', key='add_acc', size=(18, 1), **btn_font)],
        [sg.VerticalSeparator((125, 5))],
        [sg.T(' ' * 2),  sg.Button('Change Password Bank', key='bank_passwd', size=(22, 1), **btn_font)],
        [sg.VerticalSeparator((125, 5))],
        [sg.T(' ' * 19), sg.Button('Exit', size=(6, 1), **btn_font)]
    ]
...
def popupAddOrUpdate(acc='', login='', passwd=''):
    ...
    popup = sg.Window('Add' if not acc else acc, finalize=True, layout=
        [[sg.Text('Account',  size=(8, 1), **text_font), sg.Input(size=(20, 1), default_text=acc,   **text_font)],
         ...
         [sg.T(' ' * 30), sg.Button('Generate Password', size=(17, 1), key='gen', **btn_font)],
         [sg.T(' ' * 3), sg.Button('Save', size=(8, 1), **btn_font), sg.T(' ' * 9), sg.Cancel(size=(10, 1), **btn_font)]
    ])
...
def popupChangeBankPassword():
    ...
    popup = sg.Window('Change Password Bank', finalize=True, layout=
        [[sg.Text('Old Password',  size=(12, 1), **text_font), sg.Input(size=(20, 1), password_char='*', focus=True, **text_font)],
         ...
         [sg.T(' ' * 41), sg.Button('Generate Password', size=(17, 1), key='gen', **btn_font)],
         [sg.T(' ' * 5),  sg.Button('Save', size=(10, 1), **btn_font), sg.T(' ' * 13), sg.Cancel(size=(10, 1), **btn_font)]
    ])
...
def getAccounts():
    ...
    subprocess.check_call(['attrib', '+H', user_bank])
...
def popupShowAccount(acc):
    ...
    popup = sg.Window(acc, finalize=True, layout=
        [[sg.Text('Login',    size=(8, 1), **text_font), sg.Text(accounts[acc][0], text_color='burlywood1', font=('Georgia', 13), pad=(1, 5))],
         ...
         [sg.T(' ' * 18), sg.Button('Copy Password', key='c_pw', size=(14, 1), **btn_font)],
         [sg.OK(size=(24, 1), **btn_font)]
    ])
